...
function AbstractGrid(x_size, y_size, dots) {
  ...
  slf.checkForThreeSides = function (p1) {
    var incomplete = slf.checkForIncompleteSides(p1);
    if (incomplete) {
  ...
  slf.addLine = function (p1, p2) {
    ...
    if (mode == "V") {
      ...
      console.log(
        "That's odd -- we shouldn't ever get here :( Points: " + p1 + "," + p2
      );
    }
...
function FakeDots() {
  ...
  slf.toCoord = function (n) {
    return n;
  };
  slf.drawTurn = function () {
    return false;
  };
  slf.drawVictory = function () {
    return false;
  };
}
...
function Dots() {
  ...
  this.onLevelChange = function (event) {
    ...
    slf.grid.ai.level =
      slf.levelButton.options[slf.levelButton.selectedIndex].value;
  };
  ...
  this.autoDrawGrid = function () {
    var x_size = parseInt(slf.xBox.value);
    var y_size = parseInt(slf.yBox.value);
    var x_space = slf.canvas.width / (x_size + 1);
    var y_space = slf.canvas.height / (y_size + 1);
    if (x_space < y_space) {
  ...
  this.updateAnimation = function () {
    ...
    function highlightPoint(idx, x, y) {
      ...
      if (
        !cleared &&
        (slf.highlights[idx][0] != gx || slf.highlights[idx][1] != gy)
      ) {
        console.log(
          "Clear rectangle! highlight %s,%s is changing",
          slf.highlights[idx][0],
          slf.highlights[idx][1]
        );
        slf.actx.clearRect(0, 0, slf.canvas.width, slf.canvas.height);
        cleared = true;
      }
  ...
  this.onTouchEnd = function (event) {
    ...
    slf.endX =
      slf.touches[slf.touches.length - 1][0].pageX - slf.canvas.offsetLeft;
    slf.endY =
      slf.touches[slf.touches.length - 1][0].pageY - slf.canvas.offsetTop;
    slf.touches = [];
    slf.inTouch = false;
    slf.drawLine();
  ...
  this.drawLine = function (mode) {
    ...
    // 	    sx,sy,
    // 	    ex,ey);
    if (ex >= slf.grid.x_size) {
      ex = slf.grid.x_size - 1;
    }
    if (ey >= slf.grid.y_size) {
      ey = slf.grid.y_size - 1;
    }
    if (sx >= slf.grid.x_size) {
      sx = slf.grid.x_size - 1;
    }
    if (sy >= slf.grid.y_size) {
      sy = slf.grid.y_size - 1;
    }
    if (ex < 0) {
      ex = 0;
    }
    ...
    if ((ey == sy || ex == sx) && (ey != sy || ex != sx)) {
      ...
      if (mode == FINAL) {
        ...
        if (
          slf.grid.lineIsFilled(slf.grid.getCoordinates([sx, sy], [ex, ey]))
        ) {
...
function draw() {
  var dots = new Dots(); // Global...
  var sizes = getWindowSizes();
  var xratio = sizes[0] / sizes[1];
  var ydots = 8;
  var xdots = 8 * xratio;
